...
#define MXNET_OPERATOR_REGISTER_PDF(distr, pdffunc, num_parms, \
  ...
  .set_attr<nnvm::FInferShape>("FInferShape", PdfOpShape<vectorparms>) \
...
#define MXNET_OPERATOR_REGISTER_PDF_GRAD(distr, pdffunc, num_parms, vectorparms) \
  ...
  .set_attr<nnvm::FInplaceOption>("FInplaceOption", [](const NodeAttrs& attrs) \
    { std::vector<std::pair<int, int> > v = {{1, 0}, {2, 1}, {3, 2}}; v.resize(num_parms+1); return v; }) \
...
#define MXNET_OPERATOR_REGISTER_PDF1(distr, pdffunc, parm_name, parm_desc, description, vectorparms) \
